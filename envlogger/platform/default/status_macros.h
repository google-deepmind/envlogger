...
#ifndef THIRD_PARTY_PY_ENVLOGGER_PLATFORM_DEFAULT_STATUS_MACROS_H_
#define THIRD_PARTY_PY_ENVLOGGER_PLATFORM_DEFAULT_STATUS_MACROS_H_
...
//     ENVLOGGER_RETURN_IF_ERROR(Function(args...));
//     ENVLOGGER_RETURN_IF_ERROR(foo.Method(args...));
...
// For example:
//   absl::Status MultiStepFunction() {
//     ENVLOGGER_RETURN_IF_ERROR(Function(args...)) << "in MultiStepFunction";
//     ENVLOGGER_RETURN_IF_ERROR(foo.Method(args...)).Log(base_logging::ERROR)
...
//   }
//
//   ENVLOGGER_RETURN_IF_ERROR(foo()).With(TeamPolicy);
//   ENVLOGGER_RETURN_IF_ERROR(bar()).With(TeamPolicy);
//
...
//     return absl::OkStatus();
//   }
#define ENVLOGGER_RETURN_IF_ERROR(expr)                                   \
...
// Interface:
//
//   ENVLOGGER_ASSIGN_OR_RETURN(lhs, rexpr)
//   ENVLOGGER_ASSIGN_OR_RETURN(lhs, rexpr, error_expression);
//
...
//   ENVLOGGER_ASSIGN_OR_RETURN(ValueType value, MaybeGetValue(arg));
//
// Example: Assigning to an existing variable:
//   ValueType value;
//   ENVLOGGER_ASSIGN_OR_RETURN(value, MaybeGetValue(arg));
//
// Example: Assigning to an expression with side effects:
//   MyProto data;
//   ENVLOGGER_ASSIGN_OR_RETURN(*data.mutable_str(), MaybeGetValue(arg));
//   // No field "str" is added on error.
//
// Example: Assigning to a std::unique_ptr.
//   ENVLOGGER_ASSIGN_OR_RETURN(std::unique_ptr<T> ptr, MaybeGetPtr(arg));
//
...
// to ENVLOGGER_RETURN_IF_ERROR. The expression may, however, evaluate to any
// type returnable by the function, including (void). For example:
//
// Example: Adjusting the error message.
//   ENVLOGGER_ASSIGN_OR_RETURN(ValueType value, MaybeGetValue(query),
...
//
#define ENVLOGGER_ASSIGN_OR_RETURN(...)                               \
  STATUS_MACROS_IMPL_GET_VARIADIC_(                                   \
      (__VA_ARGS__, STATUS_MACROS_IMPL_ENVLOGGER_ASSIGN_OR_RETURN_3_, \
       STATUS_MACROS_IMPL_ENVLOGGER_ASSIGN_OR_RETURN_2_))             \
  (__VA_ARGS__)
...
#define STATUS_MACROS_IMPL_ENVLOGGER_ASSIGN_OR_RETURN_2_(lhs, rexpr) \
  STATUS_MACROS_IMPL_ENVLOGGER_ASSIGN_OR_RETURN_3_(lhs, rexpr, std::move(_))
#define STATUS_MACROS_IMPL_ENVLOGGER_ASSIGN_OR_RETURN_3_(lhs, rexpr,       \
                                                         error_expression) \
  STATUS_MACROS_IMPL_ENVLOGGER_ASSIGN_OR_RETURN_(                          \
      STATUS_MACROS_IMPL_CONCAT_(_status_or_value, __LINE__), lhs, rexpr,  \
      error_expression)
#define STATUS_MACROS_IMPL_ENVLOGGER_ASSIGN_OR_RETURN_(statusor, lhs, rexpr, \
                                                       error_expression)     \
  auto statusor = (rexpr);                                                   \
...
//
//   if (do_expr) ENVLOGGER_RETURN_IF_ERROR(expr) << "Some message";
//
...
#define ENVLOGGER_CHECK_OK(val) CHECK_EQ(::absl::OkStatus(), (val))

#endif  // THIRD_PARTY_PY_ENVLOGGER_PLATFORM_DEFAULT_STATUS_MACROS_H_
