...
TEST(RiegeliShardWriterTest, KStepsLegacyIndex) {
  ...
  // Write some predictable data.
  {
    ...
    writer.AddStep(ParseTextOrDie<Data>(
        R"pb(datum: { values: { float_values: 3.0 } })pb"));
    writer.AddStep(ParseTextOrDie<Data>(
        R"pb(datum: { values: { float_values: 4.0 } })pb"));
    writer.Flush();
  }

  // Check that the index contains the expected data.
  riegeli::RecordReader record_reader(RiegeliFileReader<>(index_filename, "r"));
  Data data;
  EXPECT_THAT(record_reader.ReadRecord(data), IsTrue);
  const auto& dict = data.dict().values();
  EXPECT_THAT(dict, SizeIs(2));
  EXPECT_THAT(dict, Contains(Key("episode_starts_array")));
  EXPECT_THAT(dict, Contains(Key("step_offsets_array")));

  // Check episode start indices (in step units).
  const auto episodes_decoded = Decode(dict.at("episode_starts_array").datum());
  EXPECT_THAT(episodes_decoded.has_value(), IsTrue());
  EXPECT_THAT(std::holds_alternative<xt::xarray<int64_t>>(*episodes_decoded),
              IsTrue());
  const xt::xarray<int64_t>& episodes =
      std::get<xt::xarray<int64_t>>(*episodes_decoded);
  EXPECT_THAT(episodes, ElementsAre(0));

  // Check for step offsets (in file offsets).
  const auto steps_index_decoded =
      Decode(dict.at("step_offsets_array").datum());
  EXPECT_THAT(steps_index_decoded.has_value(), IsTrue());
  EXPECT_THAT(std::holds_alternative<xt::xarray<int64_t>>(*steps_index_decoded),
              IsTrue());
  const xt::xarray<int64_t>& steps_index =
      std::get<xt::xarray<int64_t>>(*steps_index_decoded);
  EXPECT_THAT(steps_index, SizeIs(4)) << "Expected 4 steps.";
  // Check for an increasing sequence of file offsets.
  for (size_t i = 0; i + 1 < steps_index.size(); ++i) {
    EXPECT_THAT(steps_index(i), Lt(steps_index(i + 1)))
        << "Non-increasing sequence detected at index " << i << " and "
        << (i + 1);
  }

  // Check actual step data in trajectories.
  std::vector<Data> steps;
  riegeli::RecordReader trajectories_reader(
      RiegeliFileReader<>(trajectories_filename, "r"));
  Data step_data;
  while (trajectories_reader.ReadRecord(step_data)) steps.push_back(step_data);

  EXPECT_THAT(
      steps, ElementsAre(EqualsProto("datum: {values : {float_values : 1} }"),
                         EqualsProto("datum: {values : {float_values : 2} }"),
                         EqualsProto("datum: {values : {float_values : 3} }"),
                         EqualsProto("datum: {values : {float_values : 4} }")));
}
